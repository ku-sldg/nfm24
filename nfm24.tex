...
certificate's Subject field. To summarize, the IAK's good
attributes are guaranteed by the attribute check performed by the CA
and the simultaneous presence of the LAK and IAK on the TPM is guaranteed
...
\begin{enumerate}[itemsep=0pt,parsep=0pt,partopsep=0pt]
  ...
  \item The CA checks the returned nonce against the one generated in Step 6
...
Proving the IAK has good attributes proceeds
identically to the corresponding proof in the previous
section. Proving the EK certificate is valid is trivial
...
composition of Steps 1-4 and 7 using the
...
We now verify the same goal assuming only the CA is trusted to
...
executes some arbitrary sequence of commands \verb|seq1|
producing some message \verb|csr| in the OEM's intermediate state; the
OEM's initial state contains only messages it cannot generate;
the CA executes its steps on \verb|csr| and sends the challenge blob
\verb|encryptedCredential (hash (publicKey| \verb|iak)) g ek| to the OEM; the
...
the challenge blob is the same nonce in the OEM's final state.
...
correct final step (i.e., Step 7). This proof uses two important
...
We lastly show that the private
...
received CSR.  The IAK's good attributes are guaranteed by the
attribute check performed by the CA, the simultaneous presence of the
IAK and EK on the TPM is guaranteed by the EK's \verb|Restricted| attribute and the
secret contained in the challenge blob, and the EK certificate's validity
is guaranteed by the signature check on the EK
...
encouraged to ensure the CA it is issuing a certificate to a
