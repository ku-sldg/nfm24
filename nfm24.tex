...
Attestation Key (IAK) wrapped by the EK. The IAK is an AK - an ordinary, restricted, signing
key.  Like the EK, the IAK has an associated certificate generated by
the OEM CA that binds its TPM to the device.  The OEM then creates the Initial
DevID (IDevID) and the OEM's CA creates the IDevID Cert.  Initial device
identifiers are permanent, unique identifiers of TPM-containing devices, intended to be
long-lived and usable for the lifetime of the device. As such, initial
identifiers should be used sparingly outside the TPM to limit risk of
compromise.
