...
We consider two protocol execution scenarios: (i) the
certificate-requesting entity and the CA both execute their steps
correctly; and (ii) only the CA executes its steps correctly.  Because
...
conditions of the first scenario where the Owner and the CA are both
trusted to execute their steps correctly. Recall that this proof
...
Owner and the CA are trusted to execute their steps correctly.

We now attempt verify the same goal under the second scenario where
...
the EK certificate is valid. These assurances correspond with the
...
public key from the TPM Manufacturer's CA.
% The command
...
We now verify the IAK and EK are loaded on the same TPM when the OEM
and the CA are both trusted to execute their steps correctly. We
...
We now verify the same goal assuming only the CA is trusted to execute
its steps correctly.  We modify the \verb|arbitrary_CSR_production|
...
blob, and that the EK and IAK are simultaneously present is guaranteed
by the signature check on the EK certificate performed by the CA.
