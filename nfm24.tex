...
assertion about TPM-residency, key attributes, or previously-issued
certificates and provides the basis for the resulting cryptographic
evidentiary chain formed by a certificate chain. We verify that each
protocol guarantees its associated assurances.

When verifying these goals, we consider two scenarios: (i) the
certificate-requesting entity and the CA are both execute their steps
...
scenarios assume the precondition that previously-issued certificates
...
two assurances: (A) the LAK has good attributes; and (B) the LAK is
