...
protocol execution we use a labeled transition system to model
sequences of TPM and TSS command execution.  State is represented as a
...
transformations corresponding with commands executed by the OEM or
Owner CA.

\subsection{Keys and Certificates}

Our model defines a \verb|pubKey| and \verb|privKey| type for public
...
a particular attribute is set or not set. A key pair consists of a
...
parameters such as keys, nonces, certificates, and messages that
define structures an entity may use or produce. The \verb|message|
type shown in Figure~\ref{fig:message-model} is an abstract
