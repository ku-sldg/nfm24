...
A TPM key is an asymmetric key that may only be used by the creating
TPM.  Primary TPM keys are
...
TPM keys are wrapped by a parent
...
TPM keys have attributes set at creation-time restricting how they may be used.
Attributes include \verb|FixedTPM| to control duplication, \verb|Sign| and
\verb|Decrypt| to indicate use as a signing or decryption key, and
\verb|Restricted| to limit operations to TPM-generated data only.

All TPMs are shipped with a root storage key, the endorsement key
(EK).  The EK is a fixed-TPM, restricted, decryption key installed by
the TPM Manufacturer in an immutable, shielded TPM location.  The EK
has an associated certificate that binds the EK to a specific TPM.
Although not a DevID, the EK plays a significant role as a
root-of-trust in provisioning a TPM-based device identity.
