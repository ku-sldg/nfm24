...
Trust for new DevIDs certificates is inherited from an existing
certificates forming a chain-of-trust from the new DevID back to a
...
that the IAK key belongs to a specific device. The IAK certificate
acts as a trust anchor for the device and is the root node in any
chain of LAK certificates associated with the device.  The IAK and
IDevID are created by the OEM and are maintained for the device's
lifetime.

Local AK (LAK) certificates are chained from an IAK certificate
providing evidence that an LAK belongs to the same device as tie IAK.
LAK certificates and LDevIDs are created by users and serve as aliases
for the device.  A single device may have many identifying LAK
certificates and LDevIDs.  LAK certificates and LDevIDs are
short-lived and may be created, chained and removed at will by the
system owner.

In a properly configured TPM, a CA signed EK identifies it as a
legitimate trust root for its TPM.  An EK signed IAK binds the TPM to
the device where it is installed.  An IAK signed LAK binds an alias
key to the same device as the IAK.  IDevIDs identify the device while
LDevIDs are equally strong aliases for the same device.  
