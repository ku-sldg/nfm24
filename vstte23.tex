...
\begin{abstract}
  ...
  to ensure such identifiers are strongly bound to their
  devices.  Lack of such assurances calls into question the integrity
  of decisions that rely upon associating data with devices.  We
  developed models of two TCG provisioning protocols and verified they
  provide the assurances necessary to produce a cryptographic binding
  of TPM key to device.
  % guarantee the properties necessary to produce a cryptographic 
  % binding of TPM key to device.
  % We developed and verified two models of TCG provisioning protocols 
  % resulting in assurance that secure identifiers are created from a 
  % properly provisioned TPM.  
...
chain linking a DevID to a specific TPM-containing device \citep{DevIDSpec-TCG}.  
For each provisioning procedure, the TCG outlines a protocol for the CA
...
verified/synthesized attestation
environment\citep{petz2022innovations}.  
Our contributions of this research include: (i) design and implementation
...
(PCRs)~\citep{PCRAnalysis}, enhanced
authorization~\citep{EAAnalysis}, and hash-based message
authentication code (HMAC) mechanisms~\citep{HMACAnalysis}. They
utilize tools and techniques such as SAPIC~\citep{kremer2016automated}, Tamarin~\citep{meier2013tamarin}, and stateful
applied pi calculus~\citep{Milner:99:Communicating-a}, amongst others.
%
%
%
\section{Background}
...
TPM keys have attributes that are set at creation-time and restrict
how they may be used.  Attributes include \verb|FixedTPM| to control
...
Although not a DevID itself, the EK plays a significant role as a
...
residency while the second uses a command supersequence to model
...
attestation manager that produced it. LDevID certification makes them
ideal for identifying attestation managers and providing signing keys
for attestation results.  Verification results reported here allow
safe integration of LDevIDs into our verified attestation tool
