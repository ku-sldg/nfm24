...
cryptographically bound to a device creating a
DevID certificate. Certificates are chained from a trusted root
certificate to extend trust to new keys. Certificate Authorities (CAs)
associated with the TPM manufacturer, OEMs, and device owners interact
with the TPM to create an initial DevID permanently bound to the
device and collections of local DevIDs that serve as device aliases:
...
OEM and is present for the lifetime of its associated device.% In
