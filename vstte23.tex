...
Specifically, signing and decryption operations may only be performed
on the creating TPM.  TPM keys have attributes that are set at
...
AK. AK's have their \verb|Restricted| attribute set resulting in
cryptographic binding of keys and resulting signatures to the device.
...
should not be used outside the TPM to limit risk of compromise.
...
and shared externally as device identities.
...
To produce a valid certification request message, specific commands
...
generate the request is a supersequence of their correct steps. A list
is a supersequence of another list if and only if all the elements of
the second list occur in order in the first --- the elements need not
